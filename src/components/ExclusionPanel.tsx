...
function matchNode(win: WindowUIData, node: RuleNode): boolean {
    if (node.type === "group") {
...
function isExcluded(win: WindowUIData, jsonStr: string): boolean {
    const root = parseRules(jsonStr);
    return matchNode(win, root);
}

function updateNodeByPath(root: RuleNode, path: number[], newNode: RuleNode | null): RuleNode {
...
function getNodeByPath(root: RuleNode, path: number[]): RuleNode | null {
    let curr = root;
